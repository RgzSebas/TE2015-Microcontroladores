...
MAIN:
    ...
    ; GPIO CONFIGURATION
    CLRF TRISA,0 ;Configure ((INT0PPS) and 0FFh), 3, b A as output
...
;10) Load value from ((INT0PPS) and 0FFh), 3, b B to WREG. If PB4 = 0 (on-board switch), toggle ((PORTF) and 0FFh), 3, a (on-board LED).
LOOP:
    MOVF PORTB,0,0
    BTFSS WREG,4,0
    BTG LATF,3,0

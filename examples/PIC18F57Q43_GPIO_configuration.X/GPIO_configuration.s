;-------------------------------------------------------------------------------
;| TECNOÓGICO DE MONTERREY, CAMPUS MONTERREY                                   |
;| TE2015 MICROCONTROLADORES, AGOSTO-DICIEMBRE 2022                            |
;| PROFESOR: MATÍAS VÁZQUEZ PIÑÓN                                              |
...
; CONFIGURAR LAS ENTRADAS Y SALIDAS DEL MICROCONTROLADOR 18F57Q43
; EL LED DE LA TARJETA DE DESARROLLO ESTÁ CONECTADO AL PIN RF3
; EL BOTÓN DE LA TARJETA DE DESARROLLO ESTÁ CONECTADO AL PIN RB4
; ESTA INFORMACIÓN SE ENCUENTRA EN LA PÁGINA 23 DE LA HORA DE DATOS
; DE LA TARJETA CURIOSITY NANO PIC18F57Q43.
...
main:
    ...
    BCF	    TRISF,3,0		; BIT CLEAR F TRIS F BIT 3 (LED) USING ACCESS BANK
    BSF	    TRISB,4,0		; BIT SET F TRIS B BIT 4 (BUTTON) USING ACCESS BANK
    
    BANKSEL ANSELB		; SELECCIONAR BANCO ANSELB
    BCF	    ANSELB,4,1		; RB4 COMO PIN DIGITAL
    BANKSEL WPUB		; SELECCIONAR BANCO WPUB
    BSF	    WPUB,4,1		; RB4 CON RESISTENCIA PULL-UP HABILITADA

    BCF	    LATF,3,0		; ENCENDER LED CON 0 LÓGICO
loop:
    GOTO loop
END